#include "functions.h"


void pointer_swap(void** a, void** b){
    void* temp;
    temp = *a;
    *a = *b;
    *b = temp;
}

void allocate_image(image *u, int m, int n){
    u->m = m;
    u->n = n;

    u->image_data = malloc(m*sizeof(u->image_data));
    for(int i = 0; i < m; i++){
        u->image_data[i] = malloc(n*sizeof(u->image_data[i]));
    }
}

void deallocate_image(image *u){

    for(int i = 0; i < u->m; i++){
        free(u->image_data[i]);
    }
    free(u->image_data);
}


void convert_jpeg_to_image(const unsigned char *image_chars, image *u){

    for(int i = 0; i < u->m; i++){
        for(int j = 0; j < u->n; j++){
            u->image_data[i][j] = image_chars[i*u->n + j];
        }
    }
}

void convert_image_to_jpeg(unsigned char *image_chars, image *u){

    for(int i = 0; i < u->m; i++){
        for(int j = 0; j < u->n; j++){
            image_chars[i*u->n + j] = u->image_data[i][j];
        }
    }
}

void iso_diffusion_denoising_parallel(image *u, image *u_bar, float kappa, int iters, int my_rank, int num_procs){

    //allocate memory for halo vectors
    float* halo_top = malloc(u->n*sizeof(*halo_top));
    float* halo_bot = malloc(u->n*sizeof(*halo_bot));

    //set u_bar = u along the boundaries
    for(int i = 0; i < u->m; i++){
        u_bar->image_data[i][0] = u->image_data[i][0];
        u_bar->image_data[i][u->n-1] = u->image_data[i][u->n-1];
    }

    for(int j = 0; j < u->n; j++){
        u_bar->image_data[0][j] = u->image_data[0][j];
        u_bar->image_data[u->m-1][j] = u_bar->image_data[u->m-1][j];
    }



    //iso diffusion denoising loop
    for(int iter = 0; iter < iters; iter++){



        //exchaning of halo vectors
        if(num_procs > 1){
            if(my_rank == 0){
                MPI_Send(u->image_data[u->m-1], u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD);
                MPI_Recv(halo_bot, u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }

            if((my_rank % 2) == 1 && my_rank != num_procs - 1 && my_rank != 0){
                MPI_Recv(halo_top, u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(u->image_data[u->m-1], u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD);
                MPI_Recv(halo_bot, u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(u->image_data[0], u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD);
            }

            if((my_rank % 2) == 0 && my_rank != num_procs - 1 && my_rank != 0){
                MPI_Send(u->image_data[u->m-1], u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD);
                MPI_Recv(halo_top, u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(u->image_data[0], u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD);
                MPI_Recv(halo_bot, u->n, MPI_FLOAT, my_rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }

            if(my_rank == num_procs - 1){
                MPI_Recv(halo_top, u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(u->image_data[0], u->n, MPI_FLOAT, my_rank - 1, 0, MPI_COMM_WORLD);
            }
        }


        //inner image sweep
        for(int i = 1; i < u->m - 1; i++){
            for(int j = 1; j < u->n - 1; j++){
                u_bar->image_data[i][j] = u->image_data[i][j] + kappa
                                        *(u->image_data[i+1][j]
                                        + u->image_data[i-1][j]
                                       -4*u->image_data[i][j]
                                        + u->image_data[i][j+1]
                                        + u->image_data[i][j-1]);
            }
        }


        //sweep through edges
        if(my_rank != 0){
            for(int j = 1; j < u->n - 1; j++){
                u_bar->image_data[0][j] = u->image_data[0][j] + kappa
                                        *(halo_top[j]
                                        + u->image_data[1][j]
                                       -4*u->image_data[0][j]
                                        + u->image_data[0][j+1]
                                        + u->image_data[0][j-1]);
            }
        }

        if(my_rank != num_procs - 1){
            for(int j = 1; j < u->n - 1; j++){
                u_bar->image_data[u->m-1][j] = u->image_data[u->m-1][j] + kappa
                                        *(u->image_data[u->m-2][j]
                                        + halo_bot[j]
                                       -4*u->image_data[u->m-1][j]
                                        + u->image_data[u->m-1][j+1]
                                        + u->image_data[u->m-1][j-1]);
            }
        }


        //pointer swapping
        pointer_swap((void*)&u, (void*)&u_bar);

    }//end iterations



    pointer_swap((void*)&u, (void*)&u_bar);

    free(halo_bot);
    free(halo_top);

}
